...
// const paginationItems = document.querySelectorAll(".page-item a");
...
async function createPagination() {
  ...
  try {
    ...
    let pages = Math.ceil(body.length / notesOnPage); // Количество страниц пагинации
...
async function editRow() {
  ...
  const btnsDelete = document.querySelectorAll("#deleteButton"); // Кнопка DELETE
  const oldValues = ["", "", "", "", "", "", "", "", "", "", ""]; // Массив хранения старых значений
  ...
  for (let i = 0; i < btnsEdit.length; i++) {
    ...
    btnsEdit[i].addEventListener("click", function ed() {
      oldValues.insert(i, ""); // Добавление старых ел
      oldValues.insert(i, tableBody.rows[i]); // Добавление старых ел
